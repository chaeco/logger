import { LogLevel, LoggerOptions, LogEntry, LoggerEventType, LoggerEventHandler, LoggerEvent, SamplingOptions, RateLimitOptions, FilterOptions, PerformanceMetrics, LoggerFilter } from './types'
...
export class Logger {
  ...
  // 堆栈解析缓存（LRU）
  private callerInfoCache: Map<string, { file?: string; line?: number }> = new Map()
  private readonly maxCacheSize: number = 1000

  // 日志过滤器
  private filter: Required<FilterOptions> = {
    enabled: false,
    filters: [],
    mode: 'all'
  }
  ...
  private readonly levelPriority: { [key in LogLevel]: number } = {
    debug: 0,
    info: 1,
    warn: 2,
    error: 3,
  ...
  constructor(options: LoggerOptions = {}) {
    ...
    if (this.isNodeEnv && options.file?.enabled !== false) {
      this.fileManager = new FileManager(options.file)
  ...
  private checkRateLimit(): boolean {
    ...
    if (currentCount >= this.rateLimit.maxLogsPerWindow) {
      if (this.rateLimit.warnOnLimitExceeded) {
        this.emitEvent(
          'rateLimitExceeded',
          `日志限流已触发: ${currentCount}/${this.rateLimit.maxLogsPerWindow} 日志在 ${this.rateLimit.windowSize}ms 内`
        )
      }
      return false
    }
   ...
   * @internal
   */
  private cacheCallerInfo(key: string, info: { file?: string; line?: number }): void {
    // 如果缓存已满，删除最旧的项
