 ...
 * Logger 事件类型
 */
type LoggerEventType = 'error' | 'fileWriteError' | 'levelChange';
/**
 * Logger 事件处理程序
 */
...
interface AsyncWriteOptions {
    ...
    enabled?: boolean;
    /** 写入队列最大长度，默认为 5000。
     *  - 低并发（< 1000 req/s）：1000-2000 可接受
     *  - 中等并发（1000-10000 req/s）：3000-5000 推荐
     *  - 高并发（> 10000 req/s）：8000-10000 推荐 */
    queueSize?: number;
    /** 批量写入的最大条数，默认为 200。
     *  - 低并发：50-100
     *  - 中等并发：100-200
...
export type { AsyncWriteOptions, ConsoleOptions, ErrorHandlingOptions, FileOptions, FormatOptions, LogEntry, LogLevel, LoggerEvent, LoggerEventHandler, LoggerEventType, LoggerOptions };
